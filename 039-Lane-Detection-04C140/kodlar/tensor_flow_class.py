...
def losss(logits, labels):
  ...
  '''regularizers = (tf.nn.l2_loss(local3/weights) + tf.nn.l2_loss(local3/biases) +
                  tf.nn.l2_loss(local4/weights) + tf.nn.l2_loss(local4/biases)+
                  tf.nn.l2_loss(local6/weights) + tf.nn.l2_loss(local6/biases))
  loss=cross_entropy_mean+0.001*regularizers'''
...
def train():
  ...
  with tf.Graph().as_default():
    ...
    for step in xrange(100000):
      ...
      if step % 5000 == 0 or (step + 1) == 100000:
