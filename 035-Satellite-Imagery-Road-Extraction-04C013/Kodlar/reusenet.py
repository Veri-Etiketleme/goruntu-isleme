...
def UNet(args):
    ...
    f = [16, 32, 64, 128, 256]
    inputs_0 = keras.layers.Input(input_shape)
    ...
    if args.num_classes != 1:
        outputs_probs = keras.layers.Conv2D(args.num_classes, (1, 1), padding="same", activation="softmax")(u4)
    else:
        outputs_probs = keras.layers.Conv2D(1, (1, 1), padding="same", activation="sigmoid")(u4)

    input_iter = keras.layers.concatenate([inputs_0, outputs_probs[0]], axis=3)
    p0 = input_iter
    c1, p1 = down_block(p0, f[0])  # 128
    c2, p2 = down_block(p1, f[1])  # 64 -> 32
    c3, p3 = down_block(p2, f[2])  # 32 -> 16
